...
for device in devices_db.each():
    ...
    if mac_addr in devices_addr:
        ...
        addr_last.append(device.val()['last_update'])
        ...
        for ii in range(len(addr_var)):
            if delegate_global[ii] == self:
                try:
                    ...
                    if now_time - addr_last[ii] > 15:
                        addr_last[ii] = now_time
                        db.child("devices").child(addr_key[ii]).update(
                            {"temperature": str(int(splited[4])), "last_update": now_time})
...
# read data and transfer order

# read order from WHERE?
